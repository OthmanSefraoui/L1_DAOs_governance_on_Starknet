    ...
    IERC1155Receiver
{
    ...
    function receiveProposalAndVotes(
        ...
        payload[0] = votesAgainst;
        payload[1] = votesFor;
        payload[2] = votesAbstain;
        payload[3] = proposalIdLow;
        payload[4] = proposalIdHigh;

        require(
            proposalId == (proposalIdHigh << 128) + proposalIdLow,
            "Bad proposal Id"
        );

        // Consume the message from the StarkNet core contract.
        // This will revert the (Ethereum) transaction if the message does not exist.
        starknetCore.consumeMessageFromL2(l2Governor, payload);

        proposal.voteStart.setDeadline(block.number.toUint64());
        proposal.voteEnd.setDeadline(block.number.toUint64());

        _countVote(proposalId, address(0), 0, votesAgainst, "");
        _countVote(proposalId, address(1), 1, votesFor, "");
        _countVote(proposalId, address(2), 2, votesAbstain, "");
    }
